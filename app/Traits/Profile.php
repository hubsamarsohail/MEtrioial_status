...
//    main profile search data
        ...
        $userProfile = UProfile::with(['Country', 'users'])
            ...
            ->where(function ($query) use ($request , $webUser ) {
                if ($request->start_age && $request->end_age != '') {
                    ...
                    if ($request->cast) $query->Where('cast', 'LIKE', '%' . $request->cast . '%');
                    $query->where('types', '1');
                }
            })->get();
    ...
    //user single record get
    public static function getuData($user_profile_id)
    {
        $userProfile = UProfile::with('Country', 'users')->where(function ($q) use ($user_profile_id) {
            if ($user_profile_id) $q->where('user_profile_id', $user_profile_id);
        })->get();
        return $userProfile;
    }

    public static function getsingleData($user_profile_id)
    {
        $userProfile = UProfile::with('Country', 'users')->where(function ($q) use ($user_profile_id) {
            if ($user_profile_id) $q->where('user_profile_id', $user_profile_id);
        })->first();
