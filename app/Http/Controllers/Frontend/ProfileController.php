...
//            try {
                ...
                $user = WebUsers::find($web_user_id);
...
//            }
            ...
            $this->send_firebase_order_notification($user->device_token, 'Hello, This is role create!');
        ...
        try {
            if ($request->method() == 'POST') {
                ...
                {$v = validatorMessages($v, ['mClass' => 'error']);
                    ...
                    Session::flash('success', 'Record created successfully!');
//                    if (!$obj->user_image_id) $v = validatorMessages($v, ['mClass' => 'error', 'msg' => Messages::rec_error]);
//                    $v = validatorMessages($v, ['mClass' => 'success', 'msg' => Messages::rec_cretead]);
                    return redirect(route('uProfileDetail'))->withErrors($v)->withInput();
                }
            }
