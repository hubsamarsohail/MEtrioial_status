...
if (!function_exists('encodeJWT')) {
    function encodeJWT($payloads = [], $validFor = 5, $algo = 'HS256')  # Token Validity Time in Minutes
    {
        ...
        try {
            return encrypt($jwt->encode('APwdO49zgWyRlCY8epeI8RSULajlmZss4XqJK1MFC6nt1MDjh36eae1DCG8U6qyN'));
        } catch (\Exception $e) {
            return $e->getMessage();
        }
    }
...
if (!function_exists('decodeJWT')) {
    function decodeJWT($key)
    {
        ...
        try {
            $decodedKey = $decodeKey->verify('APwdO49zgWyRlCY8epeI8RSULajlmZss4XqJK1MFC6nt1MDjh36eae1DCG8U6qyN');
            if ($decodedKey == true)
                return $decodeKey;
